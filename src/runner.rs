...
pub fn run_line_buffered<W: Write>(line: &str, tl: &mut TodoList, out: &mut W) {
    let trimmed = line.trim();
    if trimmed.is_empty() { return; }
    
    if let Ok((_, q)) = parser::query(trimmed) {
        match run_query_ref(q, tl) {
            Ok(r) => { let _ = write_result(out, &r); }
            Err(e) => { let _ = writeln!(out, "Error: {}", e.0); }
        }
    }
}
...
fn run_query_ref(q: Query, tl: &mut TodoList) -> Result<QueryResultRef<'_>, QueryError> {
    match q {
        ...
        Query::Search(params) => Ok(QueryResultRef::Found(tl.search(&params))),
    }
}

fn write_result<W: Write>(out: &mut W, r: &QueryResultRef) -> std::io::Result<()> {
    match r {
        ...
        QueryResultRef::Done => out.write_all(b"done\n"),
        QueryResultRef::Found(items) => {
            let mut buffer = itoa::Buffer::new();
